...
def parse_args():
    ...
    p.add_argument("--reference-month", "-rm", default=None,
                   help="自定义参考月 YYYY-MM-01；缺省按同比（去年同期）")
    p.add_argument("--confirm-reference", "-cr", action="store_true",
                   help="活动月已与内勤确认'就按默认同比'，跳过参考月确认直接测算")
...
def main():
    ...
    out = plan(
        ...
        confirm_reference=a.confirm_reference,
    )
    if a.format == "md":
