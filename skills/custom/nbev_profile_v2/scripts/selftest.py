#!/usr/bin/env python3
"""
selftest.py — nbev_profile_v2 自检脚本（随包交付，可重复回归）
...
def main():
    ...
    print("== 汇总行过滤 / 月份语义 ==")
    profiler.query_dimension = lambda d, **k: [
        {"clientManageTypeDesc": "A", "clientManageTemperatureDesc": "中高温", "issuedClientNum": 50},
        {"clientManageTypeDesc": "合计", "clientManageTemperatureDesc": "合计", "issuedClientNum": 200},
    ] if d == "customer" else []
    r = profiler.profile(user_id="U1", dimensions=["客户"], month="2026-06-01")["results"][0]
    check("客户合计行不计入总数", "50 人" in r["summary"] and r["data"]["unknown"] == 200)
    profiler.query_dimension = lambda d, **k: [
        {"diamondScoreGroup": "双金钻", "monthOnJobHr": 184, "monthAggUndwrtNbev": 25530000},
        {"diamondScoreGroup": "合计", "monthOnJobHr": 184, "monthAggUndwrtNbev": 25530000},
    ] if d == "team" else []
    r = profiler.profile(user_id="U1", dimensions=["队伍"], month="2026-06-01")["results"][0]
    check("队伍合计行被过滤", "184 人" in r["summary"])
    try:
        normalize_month("2026-13-01"); check("13月被拒", False)
    except Exception as e:
        check("13月被拒", getattr(e, "code", "") == "MONTH_VALUE_INVALID")
