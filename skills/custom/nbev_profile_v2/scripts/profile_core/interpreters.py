...
def interpret_team(request_id, month, rows):
    ...
    # 按钻石等级聚合（同等级可能多行，做求和）；过滤接口可能返回的"合计/总计"汇总行
    _SKIP = {"合计", "总计", "小计", "total", "Total"}
    agg = {}
    for r in rows:
...
def interpret_customer(request_id, month, rows):
    ...
    values = ["A", "BC", "DEF"]
    grid = {}
    unknown = 0  # 落在已知九宫格之外的行（如接口返回的"合计"行或未知分层）
    for r in rows:
        t = r.get("clientManageTemperatureDesc", "")
        v = r.get("clientManageTypeDesc", "")
        n = int(_f(r.get("issuedClientNum")))
        if v in values and t in temps:
            grid[(v, t)] = grid.get((v, t), 0) + n
        else:
            unknown += n
    # 合计只统计已知九宫格，避免接口"合计行"导致重复计数
    ...
    if unknown:
        summary += f"（另有 {unknown} 人未落入标准九宫格，已忽略）"
